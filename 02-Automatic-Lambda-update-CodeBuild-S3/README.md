...
Ресурси, які створює `objectives.ps1`:
...
Запусти PowerShell script із папки Task 2:

```powershell
Set-Location "C:\Users\helper\OneDrive - Lizard Soft\AZURE\!!Devops_Repos\AWS Chalenges\AWS CI-CD\ci-cd-challenge\02-Automatic-Lambda-update-CodeBuild-S3"
powershell -ExecutionPolicy Bypass -File .\objectives.ps1
```
...
- `MalformedPolicyDocument`: policy-файл має бути UTF-8 без BOM; актуальний `objectives.ps1` це враховує.
...
- `ResourceNotFoundException` на `list-builds-for-project`: CodeBuild project ще не створений. Спочатку онови sandbox credentials і повторно запусти `objectives.ps1`, після успішного створення project повтори upload `build.zip`.
